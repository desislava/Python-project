...
class brick:
    def __init__(self, crush=False, points=10, line=0,
                 column=0, x_brick=0, y_brick=0, 
...
class hard_brick(brick):
    def __init__(self, crush=False, points=20, line=0, 
                 column=0, x_brick=0, y_brick=0,
...
class player:
    def __init__(self, lifes=3, score=0, 
                 list_of_scores=[], name='UNNAMED_NEWBIE'):
        self.lifes = lifes
        self.score = score
    ...
    for coords in f.readlines():
        ...
        for coord in list_coords:
            if int(coord) is 1:
                ...
                brick1.x_brick = (int(0.065*SIZE_X) + 
                                  brick1.column*brick1.color.get_width())
                brick1.y_brick = (int(0.07*SIZE_Y) + 
                                  brick1.line*brick1.color.get_height())
...
if current_player.name in [player_name_encoded.decode()
                           for player_name_encoded in database.dict.keys()]:
...
def display(movex, movey):
    ...
    while True:
        ...
        if (y_ball + ball.get_height()/2 >= 
            HANDL_POS - mouse_c.get_height()/2) \
            and (y_ball + ball.get_height()/2 < 
                 HANDL_POS - mouse_c.get_height()/2 + 1.5) \
        ...
        for brick in bricks:
            if not brick.crush:       # condition of collision
                ...
                if len(current_bricks) == len(bricks):
                    ...
                    current_player.list_of_scores.append((current_player.score, 
                                                          str(datetime.now())))
